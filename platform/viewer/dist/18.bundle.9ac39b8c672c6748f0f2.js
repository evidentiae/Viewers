...
var index_umd = __webpack_require__(1043);
...
var ConnectedCornerstoneViewport_mapDispatchToProps = function mapDispatchToProps(dispatch, ownProps) {
  ...
  return {
    onNewImage: function onNewImage(newImageData) {
...
var ConnectedCornerstoneViewport_mergeProps = function mergeProps(propsFromState, propsFromDispatch, ownProps) {
  var eventListeners = [];

  if (propsFromState.viewport) {
    var onImageRendered = function onImageRendered() {
      var viewportIndex = ownProps.viewportIndex;
      var ee = Object(state["a" /* getEnabledElement */])(viewportIndex);
      if (!ee) return;
      cornerstone_default.a.setViewport(ee, propsFromState.viewport);
    ...
    eventListeners = [{
      target: 'element',
      eventName: cornerstone_default.a.EVENTS.IMAGE_RENDERED,
      handler: onImageRendered
    }];
  }

  return _objectSpread({}, ownProps, {}, propsFromState, {}, propsFromDispatch, {
    setViewportActive: propsFromState.isActive ? undefined : propsFromDispatch.setViewportActive,
    onNewImage: !propsFromState.SOPInstanceUID ? ownProps.onNewImage : propsFromDispatch.onNewImage,
    eventListeners: eventListeners
  });
};

var ConnectedCornerstoneViewport = Object(es["b" /* connect */])(mapStateToProps, ConnectedCornerstoneViewport_mapDispatchToProps, ConnectedCornerstoneViewport_mergeProps)(index_umd_default.a);
...
function OHIFCornerstoneViewport_ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); if (enumerableOnly) symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; }); keys.push.apply(keys, symbols); } return keys; }

function OHIFCornerstoneViewport_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i] != null ? arguments[i] : {}; if (i % 2) { OHIFCornerstoneViewport_ownKeys(source, true).forEach(function (key) { OHIFCornerstoneViewport_defineProperty(target, key, source[key]); }); } else if (Object.getOwnPropertyDescriptors) { Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)); } else { OHIFCornerstoneViewport_ownKeys(source).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } } return target; }
...
function (_Component) {
  ...
  function OHIFCornerstoneViewport() {
    ...
    OHIFCornerstoneViewport_defineProperty(_assertThisInitialized(_this), "onNewImage", function (newImageData) {
      if (_this.state.viewportData.stack.currentImageIdIndex != null) {
        _this.setState(function (state) {
          return OHIFCornerstoneViewport_objectSpread({}, state, {
            viewportData: OHIFCornerstoneViewport_objectSpread({}, state.viewportData, {
              stack: OHIFCornerstoneViewport_objectSpread({}, state.viewportData.stack, {
  ...
  _createClass(OHIFCornerstoneViewport, [{
    ...
    value: function componentDidUpdate(prevProps) {
      ...
      var displaySetInstanceUIDChanged = displaySet.displaySetInstanceUID !== prevDisplaySet.displaySetInstanceUID;
      var frameIndexChanged = displaySet.frameIndex && displaySet.frameIndex !== prevDisplaySet.frameIndex;
      var sopInstanceUIDChanged = displaySet.SOPInstanceUID && displaySet.SOPInstanceUID !== prevDisplaySet.SOPInstanceUID;

      if (displaySetInstanceUIDChanged || sopInstanceUIDChanged || frameIndexChanged) {
        this.setStateFromProps();
      }
    }
  }, {
    ...
    value: function render() {
      ...
      return react_default.a.createElement(react_default.a.Fragment, null, react_default.a.createElement(src_ConnectedCornerstoneViewport, _extends({
        ...
        onNewImageDebounceTime: 700 //onNewImage={this.onNewImage} cdekar/hanging-protocol
