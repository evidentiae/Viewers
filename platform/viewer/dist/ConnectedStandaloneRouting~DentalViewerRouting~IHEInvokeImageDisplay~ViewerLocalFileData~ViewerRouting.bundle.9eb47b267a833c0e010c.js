...
var ViewportGrid_ViewportGrid = function ViewportGrid(props) {
  ...
  var displaySet = viewportData[0];

  if (displaySet) {
    var numFrames = displaySet.numImageFrames;
    displaySet.frameIndex = 0; // try to find clones of display set in studies
    // use them to fill up grid (viewport data)

    var j = 1;
    studies.forEach(function (study) {
      study.displaySets.forEach(function (set) {
    ...
    if (j == 1) {
      for (var i = 1; i < numFrames; i++) {
        ...
        var ds = lodash_clonedeep_default()(displaySet);
        ds.frameIndex = i;
        ds.displaySetInstanceUID = utils["a" /* default */].guid();
