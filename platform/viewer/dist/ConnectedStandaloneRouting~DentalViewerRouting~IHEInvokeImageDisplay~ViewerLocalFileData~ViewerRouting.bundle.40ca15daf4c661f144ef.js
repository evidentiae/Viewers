...
var ConnectedStudyBrowser_mapDispatchToProps = function mapDispatchToProps(dispatch, ownProps) {
  return {
    onThumbnailClick: function onThumbnailClick(displaySetInstanceUID) {
      console.log("onThumbnailClick"); //console.log(ownProps.studyMetadata);
      ...
      var numFrames = displaySet.numImageFrames; //console.log("numFrames in display set found");
      ...
      var displaySets = [];
      var viewports = [];
      var images = lodash_clonedeep_default()(displaySet).images;

      for (var i = 0; i < numFrames; i++) {
        var set = lodash_clonedeep_default()(displaySet); //var image = cloneDeep(displaySet.images[i]); // already cloned?
        //image._study.

        set.numImageFrames = 1;
        set.images = images[i];
        set.frameIndex = 0;
        set.displaySetInstanceUID = utils["a" /* default */].guid();
      ...
      dispatch(setViewportLayoutAndData({
        ...
        numColumns: numColumns,
        viewports: viewports
      }, displaySets)); //dispatch(setActiveViewportSpecificData(displaySet));
    }
  };
};
